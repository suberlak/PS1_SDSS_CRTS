...
Some authors (eg. \citealt{li2018}) allow the transformation to be a higher-order polynomial, but as Fig.~\ref{fig:quasarColors} shows, quasars occupy a relatively narrow region of $(g-i)$ color space, and we find that the linear fit is sufficient. Fig.~\ref{fig:offsetsPS1} shows the r-band offsets between PS1 and SDSS as a function of SDSS(g-i) color.  The same procedure of linear fit and limiting the color range is applied to PS1 gri, PTF gR, and CRTS V. The results are summarized in Table~\ref{tab:offsets}.
...
Due to shallower depth PTF and CRTS data has larger photometric uncertainties (median $\sim 0.1$ mag) for faint objects like quasars (population median 20 mag). Plotted on Fig.~\ref{fig:combinedLCerrors}, we see that SDSS and PS1 light curve segments have much smaller errors. 
...
Since S82 quasars are spectroscopically-confirmed, we use estimates of quasar physical properties, such as black hole mass, or absolute luminosity $M_{i}$, from \cite{shen2011}, based on single-epoch spectra.  $M_{i}$ in that catalog is K-corrected to the redshift of 2, $M_{i}(z=2)$, which corresponds the peak of quasar activity and requires the least extrapolation from the measured i-band \citep{richards2006}. For details, see Appendix ~\ref{app:measureBHmass}.
