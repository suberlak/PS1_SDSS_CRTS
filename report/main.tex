...
Color terms, although robust, introduce additional level of uncertainty. Thus we choose to combine SDSS r-band only with PS1 r-band, which is sufficiently similar to the SDSS that no bandpass transformation is required.  Indeed, as Fig.~\ref{fig:SDSS_PS1_offset_mag} shows,  the PS1-SDSS r-band difference less than $0.01$ mag (1\% level) up to $r < 20.5$. The offset between these two filters is also very small (<1\% level) as a function of the  SDSS (g-i) color (Fig.~\ref{fig:SDSS_PS1_offset_gi}). %Thus we do not use the r-band ZTF nor PTF data since these would require photometric transformation to the SDSS r-band. 
