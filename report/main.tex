...
We also searched for signal of Mg\,{\sc ii} variability. Apart from  H$\beta$ and C\,{\sc iv} \citep{shen2013}, Mg\,{\sc ii}  was first used as a virial Black Hole mass estimator in quasars by \cite{mclure2002}, who found that FWHM of the Mg\,{\sc ii} doublet trails that of the  H$\beta$ line. Mg\,{\sc ii}, like  H$\alpha$  and  H$\beta$  is a permitted low-ionization line \citep{yang2019}, but on average is being emitted by gas further away from the ionizing source than the  H$\beta$, possibly at the edge of the BLR \citep{guo2019}. Already early study of an AGN NGC3783 by \cite{reichert1994} noted that the Mg\,{\sc ii} emission line lags the continuum and is less variable. \cite{ivezic2004} and \cite{macleod2012} studied the SDSS-POSS sample of quasars, and report a detection of a decrement in the data-model residuals around 2800 $\mbox{\AA}$ when plotting the residuals as a function of $\Delta t$ and $\lambda_{RF}$. We investigated the data-model residuals for $f=SF_{\infty}$ in Eq.~\ref{eq:powlawmodel}, using the fit coefficients for SDSS-PS1 from Table~\ref{tab:coefficients}. Plotting the residuals as a function of $\tau_{\mathrm{RF}}$ and $\lambda_{\mathrm{RF}}$, we find that the decrement in the median($\tau_{\mathrm{RF}}$) is at most 10\% relative to the smooth model around the 2800 $\mbox{\AA}$ Mg\,{\sc ii} region. 
