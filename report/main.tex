...
\tablecaption{Comparison of published results on correlating observed quasar light curve properties: a measure of variability amplitude A, and timescale of variability $\tau$, against the physical quasar properties: Black Hole mass $M_{\mathrm{BH}}$, Eddington ratio $f_{\mathrm{Edd}}$, bolometric luminosity $L_{Bol}$. We list the correlations as positive (+), negative (-, i.e. anti-correlation), not studied (x). We further note if the correlation is strong (!), or weak ($\sim$). \label{tab:comparison}}  

\tablehead{
\colhead{Publication} & 
\colhead{Measure of amplitude and timescale} & 
\colhead{A vs $M_{\mathrm{BH}}$} & 
\colhead{A vs $f_{\mathrm{Edd}}$} &
\colhead{A vs $L_{\mathrm{Bol}}$} &
\colhead{$\tau$ vs $M_{\mathrm{BH}}$} & 
\colhead{$\tau$ vs $f_{\mathrm{Edd}}$} &
\colhead{$\tau$ vs $L_{\mathrm{Bol}}$} &
}
\startdata
\cite{kelly2009} & DRW: $\tau$-decorrelation timescale, $\sigma$   & -- & x & !-- & + &  &  $\sim$+ \\
\enddata
