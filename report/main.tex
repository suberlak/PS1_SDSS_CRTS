...
We combine quasar photometry into a single 'master' bandpass.  We choose  $r_{\mathrm{SDSS}}$ as the target bandpass since it has the best photometry, and we calculate color terms that afford transformation from other photometric systems to SDSS. We initially translated to $\mathrm{r_{SDSS}}$ photometry from nearby filters: $\{g,R\}_{\mathrm{PTF}}$, $\{g,r,i\}_{\mathrm{PS1}}$, $V_{\mathrm{CRTS}}$, deriving color terms with the SDSS standard stars~\citep{ivezic2007}.  We focus on a 10\% subset of randomly chosen stars from the catalog, and find their CRTS (B.Sesar, priv.comm.), PS1 (from MAST \url{http://panstarrs.stsci.edu}) and PTF (IRSA PTF Object Catalog \url{https://irsa.ipac.caltech.edu/}) positional matches. 
