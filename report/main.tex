...
\end{equation}
(with $\sigma = \mathrm{SF}_{\infty} / \sqrt{2}$, $\tau$ is the characteristic timescale, $f$ is the frequency), so that $P(f) \propto f^{-2}$  at high frequencies $f > (2\pi \tau)^{-1}$, and levels to a constant value at lower frequencies\citep{kelly2014}. This can be parametrized as $P(f) \propto f^{\alpha_h}$ at high frequencies (short timescales), and  $P(f) \propto f^{\alpha_l}$  at low frequencies (long timescales), so that for a pure DRW process $\alpha_{h}=-2$ and $\alpha_{l} = 0$.  Various opinions exist about the exact value of  $\alpha_{l}$ and $\alpha{h}$ for accurate description of quasar variability,  or more broadly speaking -  AGN variability. For instance, \cite{guo2017} using a low-redshift subset of S82 quasars  finds that the low-frequency slope  $\alpha_{l}$ should not be steeper than $-1.3$.  Also, since short timescales are exquisitely probed by Kepler, \citep{aranzana2018} conducted the most extensive study of K2 AGN light curves to date, including 252 well-sampled quasars with a baseline of 80 days, and cadence of 30 mins. That study shows that while there is a range of PSD slopes from $-1$ to $-3.2$, the median of $-2.2$ is consistent with the DRW model. 
Furthermore, \citep{sun2018} found that the data quality of SDSS-like light curves is insufficient to distinguish between CAR(1) and more complex models. Likewise, \citep{zu2013}  using 223 $I$-band  OGLE quasar  light curves (baseline of  $\approx7$ years, $\approx570$ epochs) considered whether different covariance functions (powered-exponential, Mat\`ern, Kepler-exponential, Pareto exponential - see Eqs.5,6,8,9 in \citealt{zu2013}) may present a better fit, and found that while there may be small deviations, they are not significant enough to depart from a robust DRW description. 

However, \citep{kozlowski2017a} claims that based on \cite{kozlowski2016b} and \citep{caplar2017}, AGNs have PSD slopes $\alpha_{h} \leqslant -2$, which means that models assuming a DRW $\alpha_{h}=2$ would result in decorrelation timescales biased low. It is currently not possible to distinguish between close-to-DRW and DRW processes for the S82 quasars because good sampling at both low frequencies (long timescales, white noise part), and high frequencies (short timescales, red / pink noise part)\cite{kozlowski2017a}.
...
M10 fitted  this power law independently to each of the five SDSS bands, reporting the band-averaged coefficients. Fig.~\ref{fig:MacLeodShen2011} shows the example of posterior samples for $f=SF_{\infty}$ correlating the  M10 SDSS ugriz data against an updated quasar catalog of \citep{shen2011}. 


\begin{figure*}
\plotone{figs/Chelsea_ugriz_Shen2011.png}
\caption{Table 1 in M10 reported the band-averaged values for fit coefficients A,C,D for Eq.~\ref{eq:powlawmodel}.  Shown here are samples from posterior MCMC draws using M10 results for  $f=SF_{\infty}$, against \citealt{shen2011} $M_{i}$ and $M_{BH}$. Because the SDSS r-band results are different from band-averaged values,  we compare our SDSS-PS1 combined r-band results against M10 SDSS r-band only.}
\label{fig:MacLeodShen2011}
\end{figure*} 



Because we use the combined r-band data, we compare our results to M10 SDSS r-band only. We have already shown consistency of object-by-object results when using only SDSS segment in Figs.~\ref{fig:tauRF_SFinf} and~\ref{fig:celeriteCompare}. Now we show that the trends recovered using SDSS-only portions are also consistent with M10. Fig.~\ref{fig:MacLeodCeleriteShen2011sf} shows the results for $f=SF_{\infty}$ - we find that there is a larger dependence of variability amplitude on quasar luminosity, but smaller dependence on black hole mass. Fig.~\ref{fig:MacLeodCeleriteShen2011tau} concerns the $\tau$, which appears to be less correlated with both quasar luminosity and black hole mass. 


\begin{figure*}
\plotone{figs/posterior_IDL_Chelsea_Celerite_CD_MI_Z2_tau.png}
\caption{Comparison of our fits to Eq.~\ref{eq:powlawmodel}, for $f=\tau$ with SDSS only, or SDSS-PS1 combined quasar light curves, against M10 SDSS r-band.  The results from SDSS-only portion are consistent with M10 for the single band. Inclusion of the PS1 portion decreases the timescale dependence on black hole mass, but increases the luminosity dependence. This can be understood as a rotation of the plane in ($\tau$, $M_{i}$, $M_{BH}$) coordinates.  }
...
X-ray reprocessing is also consistent with  the dependence of $\hat{\sigma}$ on the Fe{\sc ii} strength, $R_{\mathrm{Fe II}}$ (the ratio of the equivalent width of   Fe{\sc ii}  between $4435-4685$ $\AA$ and H$\beta$) studied by \citep{sun2018}. They chose $R_{\mathrm{Fe  II}}$, a proxy for Eddington ratio, because it was found by \citep{shen2014}, \citep{sun2015} to be anticorrelated with $M_{BH}$
...
Variability amplitude - Eddington ratio relation has been studied by 
