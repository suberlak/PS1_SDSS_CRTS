...
The exact PSD of the quasar light curve would affect other areas of study,  such as reverberation mapping, providing some of the most accurate black hole mass estimates.  Two most widely used approaches to measuring interband lags in AGN reverberation mapping are interpolated cross-correlation function (ICCF, \citealt{gaskell1987, peterson2004})  and  light curve modeling via Markov chain Monte Carlo approach (JAVELIN, \citealt{zu2011}).  The latter assumes that higher energies drive lower energies (whether dues to x-ray reprocessing or other mechanism), and that the driving light curve is well-modeled by a DRW (with PSD equal to  $-2$ or flatter), with other light curves (at longer wavelength) are related to it via transfer function \cite{edelson2019}. This means that departure of quasar PSD from DRW, if convincingly detected, would have an impact on RM studies that use JAVELIN or other light curve modeling tools.  
