...
We have extended the baseline of Stripe82 quasars by extending the $r_{\mathrm{SDSS}}$ light curves with  the PS1 data in $g_{\mathrm{PS1}}$, $r_{\mathrm{PS1}}$, $i_{\mathrm{PS1}}$.  As the variability is assumed to originate in the accretion disk, we shift all timescales to quasar rest frame : $\tau_{\mathrm{RF}} = \tau_{\mathrm{OBS}} / (1+z)$ (in this section,  $\tau$ refers to $\tau_{RF}$). 
